# If moose is online
tellraw @a[distance=..10] [{text:"Legitermoose is online.",color:green},"\n",{text:"World: ",color:gold},{storage:player_detect,nbt:"a.response[{players:[\"Legitermoose\"]}].world",color:green}]
...
# Get World UUID
data modify storage player_detect temp.a.uuid set from storage player_detect a.response[{players:["Legitermoose"]}].world
