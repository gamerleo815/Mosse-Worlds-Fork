...
execute as @e[type=text_display,tag=moss_world] run data modify entity @s text.extra[0] set from storage player_detect temp.a.world.response.raw_name
execute as @e[type=text_display,tag=moss_world] if data storage player_detect temp.a{uuid:"lobby"} run data modify entity @s text.extra[0] set value {text:"Lobby",color:gold}

execute as @e[type=text_display,tag=moss_world] run data modify entity @s text.extra[1] set value "\"."


function code:playerdetect/moose/display_world_macro with storage player_detect temp.a.world.response
